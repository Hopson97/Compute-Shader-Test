...
struct BufferObject : public GLResource<glCreateBuffers, glDeleteBuffers>
{
    ...
    {
        glNamedBufferStorage(id, sizeof(data[0]) * data.size(), data.data(), GL_DYNAMIC_STORAGE_BIT);
    }
...
struct VertexArray : public GLResource<glCreateVertexArrays, glDeleteVertexArrays>
{
    VertexArray() = default;
    void bind() const;
    void add_attribute(const BufferObject& vbo, GLsizei stride, GLint size, GLenum type, GLuint offset);
    void reset() override;
